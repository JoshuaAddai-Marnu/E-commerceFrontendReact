...
struct ContentView: View {
    ...
    func saveTasks(){
        // MARK: 2 stages: process first compress task and then save it using userDefaults
        print("Save task _FIOR")
        guard !tasks.isEmpty else {
            print("FIOR nothing to save")
            return
        }
        
        let jsonEncoder = JSONEncoder()
        if let saveTasks = try? jsonEncoder.encode(tasks){
